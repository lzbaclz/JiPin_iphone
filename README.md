...
以可爱日常与山河旅行两套 Live 成品展示玩法，保留极拼橘红品牌色，使用更轻快的暖色背景与字体。三张 AI 山水原图分别取意漓江喀斯特、黄山松石云海和三峡江湾；三张原生 Live 素材和一张合成成品由极拼现有 `JiPinCore` 生成并经过 Apple PhotoKit 识别。
...
- 当前页面（首页、支持、隐私）统一使用用户确认的品牌名“极拼”；旧版归档作为历史快照保留。
...
双卡片区域原本仅限制最大宽度 960px，没有设置两侧自动外边距，在 1200px 容器内靠左，导致右侧留白较大。本次把下载标题、品牌介绍、卡片组合和安装帮助统一居中；两张卡片保持等宽和相同间距，手机继续纵向排列。

验证：1440px 桌面下，卡片组合及帮助区左右边距均为 240px，标题左右边距均为 340px，共同中轴为 720px；390px 手机下标题、单卡片及帮助区左右均为 20px，无横向溢出。已查看桌面实际渲染。仅修改布局 CSS 与缓存版本参数，二维码内容、链接和 App 构建均未变化，旧版归档保留。
