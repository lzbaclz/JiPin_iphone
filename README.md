...
**当前版本：4.0.13（构建 17）** · [快速开始](#快速开始) · [构建与签名](docs/BUILD.md) · [测试记录](docs/TEST-RECORD.md) · [已知边界](docs/KNOWN-ISSUES.md)
...
长图拼接支持 1–20 张照片。单张也可裁切、编辑、导出及保存草稿，随后继续添加照片；至少保留一张。相册快捷扩展支持 1–9 张，单张直接进入长图模式。详见 [4.0.13 发布说明](docs/v4.0.13-release.md)。
...
在苹果「照片」中选择 **1–9 张**，点击分享，在操作区选择「极拼」，即可使用 Action Extension 快速拼图。
