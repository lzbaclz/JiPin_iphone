...
final class CatalogTests: XCTestCase {
    ...
    func testPhotoLimits() {
        ...
        XCTAssertEqual(PhotoLimits.validate(0, mode: .longStrip), .needPhotos(minimum: 1))
        XCTAssertEqual(PhotoLimits.validate(1, mode: .longStrip), .ok)
        XCTAssertEqual(PhotoLimits.validate(2, mode: .longStrip), .ok)
        XCTAssertEqual(PhotoLimits.validate(21, mode: .longStrip), .tooMany(maximum: 20))
        ...
        XCTAssertEqual(PhotoLimits.modes(forPhotoCount: 1), [.freeform, .poster, .longStrip])
