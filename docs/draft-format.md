...
schema 1 新增可选 `exportByteLimit`（字节，独占上限，成功文件必须严格小于它）；缺省为 nil、不限制，合法范围 1000–20000000。表单的 KB 换算为 1000 字节。自定义名称保存在模板的 title，ID 仍以 custom- 开头，因此任何名称都不会触发高清四倍像素规则。换照片时沿用输出规格、清晰度、上限和原背景选择，重置源照片相关的裁切、修边与轻修。
