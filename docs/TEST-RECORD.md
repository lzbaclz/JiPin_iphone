...
## V4.0.8 模式入口修复与最终整合（2026-09-12）

- 已合并证件照完整实现与模式入口修复，在同一份代码上构建并执行 10 项界面回归，全部通过。新增 6 项模式流程覆盖真实系统选图、自由和长图直接编辑、模板和海报仅选择具体样式、通用模式预览、取消重选和照片数量不足。
- 同轮通过证件照入口／规格验证、五色／轻修／撤销，以及既有通用照片模式提示与海报预览回归。截图已人工检查，未将与上一节重合的用例重复计入不同测试总数。
- 主 App 和扩展 4.0.8 / 12 正式签名归档成功，分发权限、App Group、发布包素材和隐私键检查通过。
- 最终代码、验证证据和分发状态见 [4.0.8 发布说明](v4.0.8-release.md)。


## V4.0.9 证件照高清与草稿（2026-09-12）

41 项证件照单元测试通过，5 项证件照 UI 与 3 项旧功能 UI 通过。验证高清从原片保留细线细节、压缩标准尺寸、自定义像素、旧草稿默认高清、选择压缩持久化、保存收尾、统一草稿箱跨进程重开，以及拒绝相册权限后的替代保存入口。

界面首轮发现保存按钮因新增内容进入滚动区不可见，改为固定底部后相关测试复跑通过；完整记录和具体结果包见 [4.0.9 发布说明](v4.0.9-release.md)。原生 Mac Vision 另完成 3 张真实肖像、五底色高清及压缩输出复核，未将其宣称为真机验收。

4.0.9（13）签名归档与 IPA 验证通过，并于 18:50（英国本地时间）上传成功。18:56 核验内部测试“正在测试”、官网公开内测“正在等待审核”。两端版本一致，没有测试素材或图库回读权限进入发布包。

## V4.0.10 证件照尺寸提示（2026-09-12）

41 项单元测试、7 项不同 UI 回归通过。验证实际输出提示跟随尺寸、清晰度、撤销／重做、自定义像素及草稿恢复；实际读取二寸高清 JPEG 的 1652×2316／1200 ppi 元数据。尺寸对比页在新建与编辑入口均可打开，普通与辅助功能大字体下两个图框宽高均保持约 1.4 倍，底部对齐并可滚动查看。最终两项交互专项复跑通过，不重复计入数量。

已人工查看截图。模板及渲染算法未改动；没有将本轮模拟器结果称为真机验收。详细证据和发布状态见 [4.0.10 发布说明](v4.0.10-release.md)。

4.0.10（14）签名包于 20:44（英国本地时间）上传成功；20:50 核验内部群组“正在测试”、官网公开内测“正在等待审核”。主 App 与扩展版本一致，发布包不含测试素材、调试输入或测试用图库读取权限。

## V4.0.11 网站自定义规格（2026-09-12）

46 个不同单元用例和 7 个不同 UI 用例通过，覆盖命名、精确像素、严格文件上限、保留原背景／清零轻修、草稿及换图后的要求保留。实际生成两档 285×385 JPG 均小于 1,000,000 字节；高噪声样本验证压缩时维持像素及严格“小于”的边界。超小文件上限明确失败。

首轮界面测试发现键盘挡住下方字段，增加完成输入按钮并回归通过。已检查自定义表单截图。具体证据和分发结果见 [4.0.11 发布说明](v4.0.11-release.md)。

4.0.11（15）签名归档、IPA 导出和校验通过，于 23:40（英国本地时间）通过 Xcode 上传成功。网页账号登录失效，Apple 处理结果、内部可更新状态及外部测试配置尚待登录后核验。本轮未改变正式商店审核。

## V4.0.12 首页顺序（2026-09-13）

证件照卡片移至贴纸试贴上方，两个原有按钮定义逐字保持。复用证件照入口／自定义尺寸与贴纸试贴入口两项 UI 测试，全部通过；未添加仅镜像排列顺序的测试。结果位于 `build/HomePriority412/UI.xcresult`，发布信息见 [4.0.12 记录](v4.0.12-release.md)。

4.0.12（16）签名包于 2026-09-13 01:47（英国本地时间）上传成功。网页登录仍未恢复，TestFlight 处理与群组可用状态尚待核验。

## V4.0.13 单张长图（2026-09-17—18）

5 项核心测试通过，验证照片数量规则、单张横竖长图实际 JPEG 与分页导出、草稿保存／重开／撤销／添加、单张 Live 原生配对与动态帧差异。8 项 UI 通过，覆盖两类单张选图入口、相册保存和草稿恢复、模板数量保护、多张长图、单张快捷扩展分享、空照片保护、导入失败确认及超量取消。已人工查看单张草稿恢复后的截图。

结果：`build/SingleStrip413/Unit.xcresult`、`UI.xcresult`、`FinalUI.xcresult` 与 `UIReview/`。未新增真机相册宿主扩展验收。版本与分发见 [4.0.13 记录](v4.0.13-release.md)。
