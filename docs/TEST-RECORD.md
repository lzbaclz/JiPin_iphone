...
## V4.0.6 长图导出画质与顶部布局（2026-09-11）

- 共 78 个不同用例最终全部通过：64 核心、14 个不同 UI 流程；新增两条布局流程在 iPhone 17 / iPhone 16e、iOS 26.3.1 上均通过。
- 验证三 Live 两静态混拼的横竖原尺寸照片、细线细节保留、1440×8855 奇数高度、高清/标准区分、20 张长图尺寸限制。Photos 实际保存和原尺寸 Live 回读通过，动态和照片像素分别核验。
- 回归现有 Live 时长切换、取消、关闭重开、声音、旋转、大素材、九路动态、草稿、静态导出和长图手势。
- 发布包 4.0.6 / 10 的分发签名、App Group 和隐私用途键均已核验；模拟器专项全图库读取用途说明仅存在于测试宿主副本。
- 结果与按用例汇总：`build/Validation-export-quality/`；截图：`docs/version-screenshots/v4.0.6/`；详见 [4.0.6 发布说明](v4.0.6-release.md)。


## 长图贴图使用当前可见区域（2026-09-11）

- 新增 3 条核心用例：每次插入重新读取可见中心，已有贴图/撤销重做/草稿保留位置，导入装饰图及形状覆盖，以及无效视口回退。
- 新增 3 条 UI 流程在 iPhone 17 和 iPhone 16e / iOS 26.3.1 上均通过：顶部新增后立即拖动、纵向滚动及 144% 放大后连续添加、横向滚动及放大后添加。
- 另回归贴纸正方形像素边界、草稿与模式复制、单指长图浏览、长按交换。10 个不同用例全部通过。
- 此次更新已验证代码；没有修改版本号或上传新的 TestFlight 构建。细节与截图见 [位置改进说明](sticker-viewport-placement.md)。


## V4.0.7 App Store 复审（2026-09-11）

- 基于已验证的长图贴图定位修复打包，主 App 与扩展均为 4.0.7 / 11；正式编译、签名和分发权限校验通过。
- 已将用户提供的完整真机视频附于审核沟通及审核信息，并回复 Guideline 2.1 的六项问题。私人视频和抽帧没有纳入代码仓库或官网。
- App Store 正式提交更新为构建 11，重新提交成功，状态“等待审核”，审核通过后自动发布。见 [4.0.7 发布说明](v4.0.7-release.md)。


## V4.0.8 证件照本机验证（2026-09-12）

完整回归与最后编辑会话专项去重后，192 项单元测试中 190 通过、2 项既有 PhotoKit 回读专项因模拟器读取权限跳过、0 失败；证件照 6 项和原功能 4 项 UI 全部通过。不同测试合计 202 项，200 通过、2 跳过，未把重复运行的用例重复计数。

新增单元测试覆盖 17 项模型／存储、14 项像素渲染／编码、6 项异步编辑会话。最后专项验证了分析中切色、提亮和改模板不会丢失自动构图，手动裁切不会被迟到分析覆盖，撤销保留正确初始构图。保存拒绝权限时失败文字与设置／文件入口保持可见；保存成功则回编辑器。

原生 Mac Catalyst 用相同 Vision 与渲染代码验证 51 张真实照片（46 单人蒙版、5 多人拒绝、0 处理错误）；255 次五色 JPEG 输出均通过精确像素、300 ppi、无 GPS 检查。已检查真实发型、眼镜、耳朵及轻修上限，修复亮度高光过曝、透明 PNG 合成白洞与宽发型构图问题。复杂背景仍可能需手工修边。本轮没有声称完成计划中全部分层样本质量比例或真机性能验收。

当前模拟器的人像分割返回 `E5RT is not supported`，实际界面提供保留原底及重试，不伪造算法成功。显式插画/蒙版 fixture 只用于可重复 UI 测试。iPhone 15 Pro 在验证时锁定，手机端实际体验及 iPhone 12 性能仍待测试。

证据与边界见 [4.0.8 证件照记录](v4.0.8-id-photo.md)；正式归档、上传和模式入口改动的最终整合验证由该版本发布记录补充。
